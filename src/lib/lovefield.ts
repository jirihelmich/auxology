...
function buildSchema(): lf.schema.Builder {
  const schemaBuilder = lf.schema.create('auxology', 20);
  ...
  schemaBuilder.createTable('Person')
    ...
    .addColumn('workplace', lf.Type.STRING)
    .addNullable([
      'gender', 'titlePrefix', 'firstName', 'lastName',
      'firstName', 'lastNameSearchable', 'firstNameSearchable', 'lastName',
      'titlePostfix', 'email', 'phone', 'birthNumber', 'birthDate',
      'weight', 'length', 'headCircumference', 'description', 'workplace',
    ])
    .addPrimaryKey(['id'], true)
...
export function getDB(): Promise<lf.Database> {
  if (!dbPromise) {
    dbPromise = buildSchema().connect();
  }
  return dbPromise!;
}
